...
public class FraudDetectedConsumer {
    ...
    private void verifySignatureOrFail(FraudDetected event) {
        ...
        if (!valid) {
            log.error("Invalid HMAC signature on FraudDetected: eventId={}", event.getEventId());
    ...
    @KafkaListener(
          ...
          containerFactory = "fraudDetectedListenerContainerFactory"
    )
    public void consume(FraudDetected event, Acknowledgment ack) {
        log.debug("Received FraudDetected: transaction={}", event.getTransactionId());
